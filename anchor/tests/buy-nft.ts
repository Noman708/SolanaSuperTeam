...
async function initializeMarketplace() {
  ...
  it("Buy NFT Successfully", async () => {
    try {
      ...
      const initialSellerBalance = await provider.connection.getBalance(wallet.publicKey);
      const initialCreatorBalance = await provider.connection.getBalance(creator.publicKey);
      const initialTreasuryBalance = await provider.connection.getBalance(treasuryWallet);

      console.log("Initial Balances:");
      console.log(`Buyer: ${initialBuyerBalance / LAMPORTS_PER_SOL} SOL`);
      console.log(`Seller: ${initialSellerBalance / LAMPORTS_PER_SOL} SOL`);
      console.log(`Creator: ${initialCreatorBalance / LAMPORTS_PER_SOL} SOL`);
      ...
      await program.methods
        ...
        .accounts({
          ...
          buyerTokenAccount,
          treasuryWallet,
          creatorWallet: creator.publicKey, // Thêm creator wallet
          systemProgram: SystemProgram.programId,
          tokenProgram: TOKEN_PROGRAM_ID,
          associatedTokenProgram: ASSOCIATED_TOKEN_PROGRAM_ID,
          rent: anchor.web3.SYSVAR_RENT_PUBKEY,
      ...
      const finalSellerBalance = await provider.connection.getBalance(wallet.publicKey);
      const finalCreatorBalance = await provider.connection.getBalance(creator.publicKey);
      const finalTreasuryBalance = await provider.connection.getBalance(treasuryWallet);

      // Calculate expected amounts
      const price = nftPrice.toNumber();
      const feeAmount = price * 0.025; // 2.5% fee
      const royaltyAmount = price * 0.05; // 5% royalty
      const sellerAmount = price - feeAmount - royaltyAmount;

      console.log("\nFinal Balances:");
      console.log(`Buyer: ${finalBuyerBalance / LAMPORTS_PER_SOL} SOL`);
      console.log(`Seller: ${finalSellerBalance / LAMPORTS_PER_SOL} SOL`);
      console.log(`Creator: ${finalCreatorBalance / LAMPORTS_PER_SOL} SOL`);
      console.log(`Treasury: ${finalTreasuryBalance / LAMPORTS_PER_SOL} SOL`);

      console.log("\nPayment Verification:");
      console.log(`Seller received: ${(finalSellerBalance - initialSellerBalance) / LAMPORTS_PER_SOL} SOL (Expected: ${sellerAmount / LAMPORTS_PER_SOL} SOL)`);
      console.log(`Creator received: ${(finalCreatorBalance - initialCreatorBalance) / LAMPORTS_PER_SOL} SOL (Expected: ${royaltyAmount / LAMPORTS_PER_SOL} SOL)`);
      console.log(`Treasury received: ${(finalTreasuryBalance - initialTreasuryBalance) / LAMPORTS_PER_SOL} SOL (Expected: ${feeAmount / LAMPORTS_PER_SOL} SOL)`);
      console.log(`Buyer spent: ${(initialBuyerBalance - finalBuyerBalance) / LAMPORTS_PER_SOL} SOL (Expected: ${price / LAMPORTS_PER_SOL} SOL)`);

      console.log("\n✅ Mua NFT và trả royalty thành công!");
    } catch (error) {
      console.error("\n❌ LỖI KHI MUA NFT:");
      logError(error);
      throw error;
  ...
  // Sửa test case "Test Buy NFT with Insufficient Balance"
    ...
    try {
      ...
      await program.methods
        ...
        .accounts({
          ...
          treasuryWallet,
          creatorWallet: creator.publicKey, // Thêm dòng này
          systemProgram: SystemProgram.programId,
          tokenProgram: TOKEN_PROGRAM_ID,
          associatedTokenProgram: ASSOCIATED_TOKEN_PROGRAM_ID,
          rent: anchor.web3.SYSVAR_RENT_PUBKEY,
      ...
      // Sửa lại phần check error
      if (!error.toString().includes("Số dư không đủ để mua NFT")) {
        throw error;
      }
  ...
  // Sửa test case "Test Buy Own NFT"
  it("Test Buy Own NFT", async () => {
    ...
    try {
      ...
      // Sửa lại phần check error
      if (!error.toString().includes("Không thể mua NFT của chính mình")) {
        throw error;
      }
